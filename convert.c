...
PyObject *genResult(struct Result *rst)
{
    ...
    if(rst->re_signum)
        PyDict_SetItemString(rst_obj,
            "re_signum", PyLong_FromLong(rst->re_signum));
    if(rst->re_call != -1)
        PyDict_SetItemString(rst_obj,
            "re_call", PyLong_FromLong(rst->re_call));
    if(rst->re_file)
        PyDict_SetItemString(rst_obj,
            "re_file", PyString_FromString(rst->re_file));
        
    return rst_obj;
}

char * const* genRunArgs(PyObject *args_obj)
{//generate the argsments for exec*
    PyObject *arg;
    const char **args;
    int len, i;
